  ...
  if (eng) {
    if (PGAMMA) {
      ...
      eng.innerHTML =
        ...
        (PGAMMA.market_regime ? `<br><span style="opacity:.65;font-size:12px">🌐 market (Nifty): ${PGAMMA.market_regime === "runs" ? `⛰️ <b>runs</b> — squeeze ON${(PGAMMA.runs_via || []).length ? ` <span style="opacity:.8">(${PGAMMA.runs_via.join(" · ")})</span>` : ""}` : "🥣 <b>sticky</b> — squeeze paused (pins only)"}</span>` : "") +
