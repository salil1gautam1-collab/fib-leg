...
for tk in stocks:
    ...
    for frac in (0.12,0.35,0.58,0.82):     # 4 trades spread across the 11 years
        ...
        # swing pivots inside the visible window (drawn as dots + a faint zigzag)
        win=[p for p in pivs if s<=p.index<en]
        pv=[{"time":int(b2[p.index].ts.timestamp()),"price":round(p.price,2),
             "kind":"high" if p.kind is PivotType.HIGH else "low"} for p in win]
        # nearest mountain (swing high) and valley (swing low) that already existed at the leg origin
        prevHigh=next((round(p.price,2) for p in reversed(pivs) if p.index<lg.start_index and p.kind is PivotType.HIGH),None)
        prevLow =next((round(p.price,2) for p in reversed(pivs) if p.index<lg.start_index and p.kind is PivotType.LOW),None)
        out.append({"title":f"{tk} {t.side.value.upper()}  {lg.start_ts.date()}",
          ...
          "retr382":round(lg.retracement(0.382),2),
