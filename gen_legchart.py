...
picks=[("TCS",0),("TCS",-1),("HDFCBANK",5),("RELIANCE",8),("RELIANCE",-1)]
cache={}; out=[]
for tk,which in picks:
    if tk not in cache:
        b1=feeds.csv_dir_series(DIR,tk); b2=feeds.resample(b1,120)
        eng,_=scan._run_tf({tk:b1},True,120,cfg(),"book382",1,5)
        cache[tk]=(b2,eng[tk])
    b2,e=cache[tk]
    tr=e.trades
    if not tr: continue
...
print("wrote",len(out),"charts;",sum(len(c['bars']) for c in out),"total bars")
