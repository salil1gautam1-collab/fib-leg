"""Final filter stack on the NEW 0.618 leg base. 2H book382, lock-at-B, re_anchor 0.618.
Does A+ / pin still add on top of M/W+trend now? Train/test/full @0.10%."""
...
print(f"{len(tickers)} stocks, 2H book382, lock-at-B, re_anchor 0.618 — final filter stack\n", flush=True)
...
def cfg(**kw):
    ...
    return c
CFG = {
    "M/W+trend":            cfg(require_mw=True, require_htf=True),
    "+A+ (loose)":          cfg(require_mw=True, require_htf=True, require_aplus=True),
    "+pin (M/W|pin)":       cfg(require_mw=True, reversal_pin=True, require_htf=True),
    "+A+ +pin":             cfg(require_mw=True, reversal_pin=True, require_htf=True, require_aplus=True),
    "plain (no filter)":    base(),
}
