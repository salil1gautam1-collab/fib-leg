...
Run:  python swing_deep13_revfib.py "fibleg/data/Stocks_data"
"""
...
VARIANTS = ("BASE", "REV38", "REV50", "REV618", "LADDER")
REVF = {"REV38": 0.382, "REV50": 0.5, "REV618": 0.618}
...
def sim_ladder(b1, j, entry, stop, top, cap):
            ...
            for j in range(j0, j1):
                ...
                for fib in list(fibs):
                    ...
                    for L in LEVELS:
                        ...
                        res = {}
                        base_tgt = fib["e"] - DEEP[(TF, L)] * fib["rng"]
                        res["BASE"] = sim(b1, j + 1, level, stop, base_tgt, CAP)
                        for vn, frac in REVF.items():
                            res[vn] = sim(b1, j + 1, level, stop,
                                          level + frac * (fib["e"] - level), CAP)
